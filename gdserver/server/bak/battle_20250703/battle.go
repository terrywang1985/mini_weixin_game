...
func (w *ECSWorld) Run() {
	...
	for {
		...
		case t := <-ticker.C:
			...
			w.UpdateTargeting()
			w.UpdateAttacks(now)
			w.UpdateMovement(tick, now)
			w.CleanupEntities()

			frame := w.GenerateFrame(t.UnixMilli())
			frameCounter++
...
func (w *ECSWorld) HandlePlaceCard(cmd CmdPlaceCard) {
	...
	if !isValidPosition(posX, posY) {
		slog.Error("invalid position: ", "x", posX, "y", posY)
		return
	}
	// 站位区域限制
	if cmd.Owner == 1 && posY >= 300 {
		slog.Error("Player 1 cannot place card in opponent area", "y", posY)
		return
	}
	if cmd.Owner == 2 && posY < 300 {
		slog.Error("Player 2 cannot place card in opponent area", "y", posY)
		return
	}
	...
	// hp
	w.hps[e] = &HPComponent{HP: float64(cmd.Card.Hp)}
	// owner
	w.owners[e] = &OwnerComponent{PlayerID: cmd.Owner}
	// cardInfo
	w.cardInfos[e] = &CardInfoComponent{CardID: cmd.Card.Id, UnitType: UnitCard}
	// attack
	w.attacks[e] = &AttackComponent{
		Attack:      float64(cmd.Card.Attack),
		AttackSpeed: time.Duration(cmd.Card.AttackSpeed) * time.Millisecond,
		LastAttack:  time.Now(),
		AttackRange: float64(cmd.Card.AttackRange),
		MP:          100,
		MPCost:      10,
		MPRegen:     1,
	}
	// movement
	w.movements[e] = &MovementComponent{MoveSpeed: float64(cmd.Card.MoveSpeed)}
	// target
	w.targets[e] = &TargetComponent{Target: 0}
	// skill
	sc := &SkillComponent{Skills: make(map[uint64]*Skill)}
	for _, ms := range cmd.Card.MagicSkill {
		s := &Skill{Skill: *ms}
	...
	w.skills[e] = sc
	// growth
	w.growth[e] = &GrowthComponent{Level: int(cmd.Card.Growth.Level), XP: 0}
	// effects
	w.effects[e] = nil
	// collision
	w.collisions[e] = &CollisionComponent{ShapeType: CollisionCircle, Radius: 20}

	slog.Info("[Server] PlaceCard", "card", cmd.Card.Id, "entity", e, "position", cmd.PosX, cmd.PosY, "owner", cmd.Owner)
...
func (w *ECSWorld) ApplyDamage(target Entity, damage float64, attacker Entity) {
	if hp, ok := w.hps[target]; ok {
		hp.HP -= damage
		slog.Info("ApplyDamage", "attacker", attacker, "target", target, "damage", damage, "target_hp_lost", hp.HP)
		if hp.HP <= 0 {
			w.pendingRemoval = append(w.pendingRemoval, target)
			//log.Printf("Entity %d marked for removal (HP <= 0)", target)
			slog.Info("Entity marked for removal, HP<=0", "entity", target)
...
func (w *ECSWorld) UpdateAttacks(now time.Time) {
	...
	for e, atk := range w.attacks {
		...
		if dist <= effectiveRange && now.Sub(atk.LastAttack) >= atk.AttackSpeed && atk.MP >= atk.MPCost {
			...
			slog.Info("Entity attacked target", "Entity", e, "target", tgtComp.Target, "MPCost", atk.MPCost, "MPLost", atk.MP, "damage", damage)
			// 取目标快照
...
func BattleRoomStart() {
	...
	// 放两张卡作演示
	world.cmdCh <- CmdPlaceCard{Card: card1, PosX: 400, PosY: 200, Owner: 1}

	world.cmdCh <- CmdPlaceCard{Card: card2, PosX: 600, PosY: 400, Owner: 2}
